...
### Bug 修复
- 修复今日页显示的上下课时间与学期设置不一致：课程卡片时间改为读取学期配置（数据库 `semesters.periodTimesJson`），删除 `DateUtils` 中重复硬编码的默认时间表，仅在没有学期数据时才回退内置默认值

### 数据一致性
- 节次时间统一以数据库为唯一来源：今日页、周课表、课程详情等展示均通过 `Semester.getPeriodTimes()` 读取，不再存在与数据库脱节的第二份时间数据

### 数据与导入
- 课程导入幂等：学期内同名课程复用、相同排课时段跳过，重复导入同一 xlsx 不再产生重复课程/时段；导入时校验并丢弃越界或颠倒的节次/周次
- 清空教室、删除时段或改教室后自动回收不再被引用的教室；教室插入以唯一索引兜底，避免并发重复
- 数据库 v9：为 `schedules.roomId`、`exams.examDate` 增加查询索引（无数据变更，迁移安全）

### 提醒修复
- 关闭提醒开关后重启 App 不再重新排定任务，Worker 执行前二次校验开关
- Android 13+ 首次进入日历页主动申请通知权限，避免提醒被静默丢弃
- 提醒时间已过的考试在启动重排时跳过，不再每次启动重复通知

### 性能与体验
- 修复今日页课程卡片全部同色（单名称取色的色相恒为 0）
- 切换底部标签不再丢失日历月份状态（Pager 保留相邻页）
- 课程详情一次读取教室映射，消除逐时段查询的 N+1；考试列表课程名改为 map 查找

### 工程
- 新增 push/PR CI：编译 + 单元测试；补充 DateUtils/VersionChecker 单元测试
- 文档修正：移除未实现的"保存学期预设"描述与不存在的 CourseReminderWorker 条目

## v2.13 (2026-09-07)

### 应用内更新
- "检查更新"发现新版后可直接在应用内下载并安装，无需再跳转 GitHub 手动下载；首次安装按系统引导开启"允许安装未知应用"即可

### 周课表滑动优化
- 换用官方 HorizontalPager：每周一页，左右拖动时相邻两周并排显示、滑动全程可见，消除单页滑动方案翻页间隙的整屏黑屏闪烁；手势惯性甩动/动画打断/边界处理由系统接管

### Bug 修复
- 修复删除某一时段的课程会连带删除该课程其他时段的问题：编辑弹窗的删除只移除当前时段，课程无剩余时段时才一并移除课程记录
...
### 周课表滑动流畅性重构
- 左右滑动改全程跟手：拖动位移由普通状态同步驱动 `graphicsLayer.translationX`（GPU 合成层变换），不再每帧启动协程 + 弹簧 snapTo，拖动过程零协程分配、零重组
- 换页动画重做：松手翻页 = 旧页沿手势方向滑出屏外 → 屏外切换数据 → 新页从手势来源侧滑入（单一 X 位移动画，方向与手势一致）；删除原先"弹回 + 淡出 + 换数据 + 淡入"两套动画叠加，消除松手后的停顿与内容突现
- 换页动画进行中再次下压可打断并直接跟手；按钮连点/手势连续滑动自动衔接连续翻页（修复此前连点"下周/上周"偶发吞点击）
- 翻页触发改用滑动窗口速度估算（替代单事件速度），阈值重调：慢速平移浏览不再误翻页；首/末周边界阻尼与弹簧回弹保留

### 日历页滑动优化
- 月历左右拖动实时跟手（graphicsLayer 平移），松手过阈值甩出当前月并让新月滑入，未过阈值弹簧回弹；月份内容切换仅保留淡入淡出，不再叠加两套位移

### 渲染与全局性能
- 周课表课程块/占用格缓存 key 由 hashCode 改为对象引用：消除哈希碰撞风险与每次重组 O(n) 重算
- 设置页字体大小滑条：拖动仅本页局部 Density 预览，松手才一次性提交全局，避免每帧写全局 Density 触发全 App 重排

---

## v2.10 (2026-09-01)

### 教室改为时段级（同课程不同时段可不同教室）
- 数据模型：教室从课程级（courses.roomId）迁移到时段级（schedules.roomId），同一课程在不同时间段可有不同上课地点
- 数据库 v7→v8 迁移：schedules 新增 roomId 列，并自动把原课程教室回填到该课程全部时段（老数据不丢）
- 周课表课程方块：每个方块显示自己时段对应的教室（此前整门课程共用一个教室）
- 课程详情页：教室按时段对应展示（时间安排行显示"周X a-b节 @教室"），编辑回显取对应时段教室
- 今日课程卡片：显示当天该时段教室
- xlsx 导入：每个时段独立解析并写入各自教室（此前仅首个时段教室落库，其余丢弃）

### 界面修复
- 周课表当日高亮适配 8dp 圆角（表头行与日期行不再为方形高亮）
- 周课表表头行（周一~周日）与节次列去掉灰底，仅保留边框文字
...
### 提醒/今日/日历逻辑重构
- 课程不再系统通知提醒，仅考试保留系统通知（移除 CourseReminderWorker 与课程通知渠道）
- 今日页以"当天"为准判断课程：未开学（学期开始前）与学期已结束不显示任何课程
- 今日页空状态区分三种文案：还没开学 / 学期已结束 / 今天没有课程

### 课表导入改进
- 导入表头匹配改为按别名优先级（如"教师姓名"优先于"任课教师"），避免多别名列同时存在时选到含工号/职称的脏数据列

### 界面调整
- 周课表课程方块圆角加大（2dp → 8dp）
- 字体大小调节改为全局生效：覆盖今日/周课表/日历/设置页及弹窗（原仅设置页内预览）
...
### 周课表课程块间距
- WeekGrid: 课程块上下左右各留 2dp 间隙，课程之间不再紧贴；连堂块只在整块首尾留间隙，内部节间保持连续

### 内容区网格线去除
- WeekGrid: 移除内容区（右侧课程区域）Canvas 网格线，仅保留节次列边框与表头；课程块以间隙+颜色区分
...
- CourseDetailViewModel 补充 semester 数据，编辑对话框正确回显总周数/节数/每周天数
- 编辑保存/删除复用 WeekViewModel 逻辑，与周课表长按编辑行为一致
...
- 迁移改为先将被删行的课程引用重定向到同名保留行，再删除重复教室，最后建唯一索引
- docs/verify_migration.py 新增悬空引用断言，三条升级路径（v1/v2/v6→v7）验证均无悬空引用
- 已升级用户的手动恢复：编辑课程重新填写教室名即可（insertRoom 会自动复用同名教室）
